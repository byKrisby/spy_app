...
class PlayerAmount extends StatelessWidget {
  const PlayerAmount({required this.amount, super.key});

  final int amount;

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<PlayerAmountCubit, int>(
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: CustomDropdown(
                    ...
                    hintText: 'playerAmount'.tr(),
                    items: _buildOptions(context, amount),
                    onChanged: (value) {
                      Word(word: '', category: '');
                      context.read<PlayerAmountCubit>().changeAmount(value ?? state);
                      if (context.read<SpyAmountCubit>().state > (value ?? state)) {
                        context.read<SpyAmountCubit>().changeAmount((value ?? state) ~/ 2);
                      }
                    },
